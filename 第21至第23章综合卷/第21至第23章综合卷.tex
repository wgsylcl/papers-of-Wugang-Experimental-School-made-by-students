...
\begin{questions}{\answeringintroduction}
    ...
    \begin{subquestions}
        \subquestion 作$\angle BCA$的角平分线.
        \subquestion 记$\angle BAC=\alpha$，若点$T$是线段$AC$上的任意一点，将点$T$绕点$A$顺时针旋转$\alpha$至点$T'$，求作点$T'$.
    \end{subquestions}
    \begin{figure}[!htb]
        \raggedleft
        \subfigure[（第21题）]{
            \begin{tikzpicture}[scale=0.6]
                \draw[step=1,gray,dashed] (-3,-3) grid (4,4);
                \coordinate[label=below right:{$C$}] (C) at (0,0);
                \coordinate[label=above left:{$A$}] (A) at (4,3);
                \coordinate[label=above right:{$B$}] (B) at (0,3);
                \coordinate[label=above left:{$T$}] (T) at (1.64848,1.23636);
                \filldraw (T) circle (.1);
                \draw (A) -- (B) -- (C) -- cycle;
            \end{tikzpicture}}
    \end{figure}
    \question 在一次军事演习中，我军一架无人机正在以$100$m/s的飞行速度对一片山区执行$500$m高度低空轰炸任务，已知该飞机以水平轰炸的方式投下航空炸弹，且炸弹的下落距离$h$、水平飞行距离$x$和飞行时间$t$之间的关系如下表所示（不计空气阻力）：
    \begin{figure}[!htb]
        \centering
        \subfigure{
        ...
        \begin{tikzpicture}[>=Stealth,scale=0.5]
            ...
            \draw (-7,5) parabola bend (-7,5) (0.5,0.5);
    ...
    \begin{subquestions}
        ...
        \subquestion 如图1，连$BC$，设直线$l_1:x=t$交线段$BC$于点$E$，交抛物线于点$F$，直线$l_2:x=t+1$交线段$BC$于点$M$，交抛物线于点$N$，试讨论比较线段$EF$与线段$MN$的大小关系.
