...
\begin{questions}{\selectingintroduction}
    ...
    \twp{$x_1=3$、$x_2=4$}{$x_1=-1$、$x_2=0$}{$x_1=1$、$x_2=2$}{$x_1=-1$、$x_2=4$}
    ...
    \onp{$15^{\circ}$}{$20^{\circ}$}{$22.5^{\circ}$}{$30^{\circ}$}\par
...
\begin{questions}{\complitingintroduction}
    \question 一元二次方程$ax^{2} + 2ax + b = 0$的一次项系数为\complitingline{}，常数项系数为\complitingline{}，两根之和为\complitingline{}.
    \question 已知在一元二次方程$x^{2} - ( m^{2} - 3 )x + m = 0$中有$x_{1} + x_{2} = 2$，则$m =$\complitingline{}.
...
\begin{questions}{\answeringintroduction}
    ...
    \begin{subquestions}
        ...
        \begin{subsubquestions}
            ...
            \subsubquestion 在\circnum{1}的条件下，若$P$为线段$AC$上一点，作$PM \bot x$轴于点$M$，作$PN \bot y$轴于点$N$，求四边形$OMPN$面积的最大值.
        \end{subsubquestions}
